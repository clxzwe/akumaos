"""Hyprland rules configuration component."""
...
class RulesComponent(HyprComponent):
    """Component for generating Hyprland window rules configuration."""
    ...
    def render(self, validated_data: Any, project_root: Path) -> str:
        """Render rules Jinja2 template."""
        template_path = project_root / "generator" / "templates" / "rules.conf.j2"
        if not template_path.exists():
            template_path = project_root / "templates" / "rules.conf.j2"
        context = {}
        if template_path.exists():
            return render_template(template_path, context)
        return "# AkumaOS Hyprland window rules configuration (placeholder)\n"

    def get_output_path(
        self, project_root: Path, output_dir: Optional[Path] = None
    ) -> Path:
        """Get output destination path for rules.conf."""
        target_dir = output_dir or (Path.home() / ".config" / "hypr" / "config")
        return target_dir / "rules.conf"
